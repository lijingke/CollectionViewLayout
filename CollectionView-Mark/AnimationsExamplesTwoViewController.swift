...
class AnimationsExamplesTwoViewController: UIViewController {
    ...
    @objc fileprivate func btnAction(_ sender: UIButton) {
        ...
        case 4:
            ...
            break
        case 5:
              self.view5.reverse.toggle()
              UIView.transition(with: self.view5, duration: 0.6, options: .transitionFlipFromLeft, animations: {
                  self.view5.setNeedsDisplay()
              }, completion: nil)
        case 6:
            let label2 = UILabel(frame: self.transitionLabel.frame)
            label2.text = self.transitionLabel.text == "Hello" ? "World" : "Hello"
            label2.textColor = .white
            label2.sizeToFit()
            UIView.transition(from: self.transitionLabel, to: label2, duration: 0.8, options: .transitionFlipFromLeft) { (_) in
...
extension AnimationsExamplesTwoViewController {
